...
- Access to the GCP project (e.g., `ee-manuelcustodio11`)
...
```bash
gcloud projects add-iam-policy-binding "ee-manuelcustodio11" \
  --member="serviceAccount:github-actions-rle@ee-manuelcustodio11.iam.gserviceaccount.com" \
...
```bash
gcloud projects describe ee-manuelcustodio11 --format="value(projectNumber)"
```
...
gcloud iam service-accounts add-iam-policy-binding \
  "github-actions-rle@ee-manuelcustodio11.iam.gserviceaccount.com" \
  --project="ee-manuelcustodio11" \
...
| `GCP_SERVICE_ACCOUNT` | `github-actions-rle@ee-manuelcustodio11.iam.gserviceaccount.com` |
...
```bash
python scripts/rasterize_ecosystem_to_cog.py --project ee-manuelcustodio11
```
...
- **enables the `storage.googleapis.com` API** and **creates a public-read bucket** `gs://ee-manuelcustodio11-rle-cogs` if it does not already exist;
...
```
Caller does not have required permission to use project ee-manuelcustodio11.
